...
表示最大的打开文件描述符(比如打开socket，读文件)的数量
...
nagle算法是否启动 ，（默认）false开启了nagle算法，  angle算法表示是否延迟发送小的数据包，等到他攒一批再发送
...
* 用来分配 ByteBuf， ctx.alloc()
  ...
  我们io操作默认读到的的ByteBuf是强制直接内存 不能通过 系统参数修改
...
![](
https://edu-1395430748.oss-cn-beijing.aliyuncs.com/images/imgs/20220218195137.png)
...
Tags:使用泛型通配符的容器只能去不能放  ，特例可以放null  

最后可能 异常信息太长查过了我们的半包粘包处理器 帧的最大值

不能步所有异常信息发送回客户端 修改RpcRequestMessageHandler.java
...
//7 绑定端口 (main线程中 doBind（）方法会添加 异步监听事件doBind（）)
...
![](
https://edu-1395430748.oss-cn-beijing.aliyuncs.com/images/imgs/20220220221631.png)

HeadContext（通道的head）中会执行ctx.fireChannelActive(); 
...
![](
https://edu-1395430748.oss-cn-beijing.aliyuncs.com/images/imgs/0032.png)
...
![](
https://edu-1395430748.oss-cn-beijing.aliyuncs.com/images/imgs/20220221194155.png)
...
![](
https://edu-1395430748.oss-cn-beijing.aliyuncs.com/images/imgs/20220221205426.png)

这里面的NioSocketChannel注册到seletor并关注read事件 过程和NioServerSocketChannel注册到seletor并关注accept过程类似，并且会触发初始化handler，和执行active事件
...
private void register0(ChannelPromise promise) {
            try {
                ...
                //触发handler的init事件进行初始化 
                //NioServerSocketChannel netty自定义了init事件在ServerBootstrap的 init（）中 主要是添加了一个handler
                //NioSocketChannel 对应的init就是我们自己写netty中childHandler（）的初始化ChannelInitializer
                pipeline.invokeHandlerAddedIferNeeded();
                
				//如果promise设置了Listener就会触发
                safeSetSuccess(promise);
                pipeline.fireChannelRegistered();
                // Only fire a channelActive if the channel has never been registered. This prevents firing
                // multiple channel actives if the channel is deregistered and re-registered.
                //触发active事件 主要作用是给SocktChannel关注read事件或者 ServerSocketChannel关注accept事件
                if (isActive()) {
                    if (firstRegistration) {
                        pipeline.fireChannelActive();
                    } else if (config().isAutoRead()) {
